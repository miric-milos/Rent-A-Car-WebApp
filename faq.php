<?php
session_start();

if(isset($_SESSION['korisnik'])){
    $ulogovan=unserialize($_SESSION['korisnik'])[0];
}

?>

<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <meta http-equiv="X-UA-Compatible" content="ie=edge">
    <title>Često Postavljana Pitanja</title>

    <!-- Stylesheets -->
    <link rel="stylesheet" href="css/navbar.css">
    <link rel="stylesheet" href="css/footer.css">
    <link rel="stylesheet" href="css/faq.css">

    <!-- Bootstrap CSS -->
    <link rel="stylesheet" href="https://stackpath.bootstrapcdn.com/bootstrap/4.3.1/css/bootstrap.min.css"
        integrity="sha384-ggOyR0iXCbMQv3Xipma34MD+dH/1fQ784/j6cY/iJTQUOhcWr7x9JvoRxT2MZw1T" crossorigin="anonymous">
    <link rel="stylesheet" href="font-awesome/css/all.css">
    <link rel="stylesheet" href="css/navbar.css">


</head>

<body>
    <!-- NAV -->

    <nav class="navbar navbar-expand-lg navbar-dark sticky-top pt-3 pb-3">
        <a class="navbar-brand mr-5 ml-3" href="index.php">RENT VISER</a>
        <button class="navbar-toggler" type="button" data-toggle="collapse" data-target="#navbarSupportedContent"
                aria-controls="navbarSupportedContent" aria-expanded="false" aria-label="Toggle navigation">
            <span class="navbar-toggler-icon"></span>
        </button>

        <div class="collapse navbar-collapse" id="navbarSupportedContent">
            <ul class="navbar-nav mr-auto mb-3 mb-md-0">
                <li class="nav-item dropdown ml-3">
                    <a class="nav-link dropdown-toggle" href="ponude.php" role="button" aria-haspopup="true"
                       aria-expanded="false">
                        RENT A CAR VOZILA
                    </a>
                </li>
                <li class="nav-item dropdown ml-3 ">
                    <a class="nav-link dropdown-toggle" href="onama.php" role="button" aria-haspopup="true"
                       aria-expanded="false">
                        O NAMA
                    </a>
                </li>
                <li class="nav-item dropdown ml-3">
                    <a class="nav-link dropdown-toggle" href="kontakt.php" role="button" aria-haspopup="true"
                       aria-expanded="false">
                        KONTAKT
                    </a>
                </li>
                <li class="nav-item dropdown ml-3 active">
                    <a class="nav-link dropdown-toggle" href="faq.php" role="button" aria-haspopup="true"
                       aria-expanded="false">
                        FAQ
                    </a>
                </li>
            </ul>
            <?php

            if (isset($_SESSION['korisnik'])) {
                if($ulogovan['status']=="korisnik"){
                    echo "<div class=\"btn-group\">
                  <a href='korisnik.php' type=\"button\" class=\"btn btn-dark\">" . $ulogovan['username'] . "</a>
                  <button type=\"button\" class=\"btn btn-dark dropdown-toggle dropdown-toggle-split\" data-toggle=\"dropdown\" aria-haspopup=\"true\" aria-expanded=\"false\">
                    <span class=\"sr-only\">Toggle Dropdown</span>
                  </button>
                  <div class=\"dropdown-menu dropdown-menu-right\">
                    <a class=\"dropdown-item\" href=\"korisnik.php#profil\">Moj profil</a>
                    <a class=\"dropdown-item\" href=\"korisnik.php#rezervacija\">Rezervacije</a>
                    <div class=\"dropdown-divider\"></div>
                    <a class=\"dropdown-item\" href=\"index.php?odjava\">Odjavite se</a>
                  </div>
                </div>
                </div>";
                }
                else{
                    echo "<div class=\"btn-group\">
                  <a href='administracija.php' type=\"button\" class=\"btn btn-dark\">" . $ulogovan['username'] . "</a>
                  <button type=\"button\" class=\"btn btn-dark dropdown-toggle dropdown-toggle-split\" data-toggle=\"dropdown\" aria-haspopup=\"true\" aria-expanded=\"false\">
                    <span class=\"sr-only\">Toggle Dropdown</span>
                  </button>
                  <div class=\"dropdown-menu dropdown-menu-right\">
                    <a class=\"dropdown-item\" href=\"administracija.php#automobili\">Automobili</a>
                    <a class=\"dropdown-item\" href=\"administracija.php#selLokacije\">Lokacije</a>
                    <a class=\"dropdown-item\" href=\"administracija.php#selNalozi\">Nalozi</a>
                    <a class=\"dropdown-item\" href=\"korisnik.php#profil\">Profil i rezervacije</a>
                    <div class=\"dropdown-divider\"></div>
                    <a class=\"dropdown-item\" href=\"index.php?odjava\">Odjavite se</a>
                  </div>
                </div>
                </div>";
                }

            } else {
                echo "<a data-toggle=\"modal\" data-target=\"#loginModal\" class=\"ml-3 ml-md-0\" style=\"color: white; text-decoration: none;\"><i class=\"far fa-user-circle\" style=\"font-size: x-large\"></i></a>";
            }
            ?>

        </div>
    </nav>

    <!-- NAV END -->


    <div id="naslov">
        <h1 class="display-4">Najčešće postavljena pitanja</h1>
    </div>

    <div id="pitanja">
        <div class="container">
            <div class="row">
                <div class="col">
                    <label for="accordionExample">
                        <h1 class="display-4 sub-heading">Placanje</h1>
                    </label>
                    <div class="accordion" id="accordionExample">
                        <div class="card">
                            <div class="card-header" id="headingOne">
                                <h2 class="mb-0">
                                    <button class="btn btn-link" type="button" data-toggle="collapse"
                                        data-target="#collapseOne" aria-expanded="true" aria-controls="collapseOne">
                                        Kojin nacini placanja su prihvatljivi?
                                    </button>
                                </h2>
                            </div>

                            <div id="collapseOne" class="collapse show" aria-labelledby="headingOne"
                                data-parent="#accordionExample">
                                <div class="card-body">
                                    Validna kreditna kartica (Visa, AMEKS ili Mastercard) na ime glavnog vozača mora biti prisutna prilikom preuzimanja kako
                                    bi se unajmilo vozilo. Unapred plaćene, Electron i Debitne kartice neće biti prihvaćene. Imajte na umu da kreditna
                                    kartica mora biti validna najmanje šest meseci nakon datuma iznajmljivanja da bi se mogla obraditi.
                                </div>
                            </div>
                        </div>
                        <div class="card">
                            <div class="card-header" id="headingTwo">
                                <h2 class="mb-0">
                                    <button class="btn btn-link collapsed" type="button" data-toggle="collapse"
                                        data-target="#collapseTwo" aria-expanded="false" aria-controls="collapseTwo">
                                        Da li je depozit obavezan?
                                    </button>
                                </h2>
                            </div>
                            <div id="collapseTwo" class="collapse" aria-labelledby="headingTwo"
                                data-parent="#accordionExample">
                                <div class="card-body">
                                    Da. Pored zaostalih naknada za najam. Sigurnosni depozit primenjuje se na sva vozila Mini, Ekonomske i
                                    Srednje klase. Za sve veće klase potreban je sigurnosni depozit u odredjenom iznosu.
                                </div>
                            </div>
                        </div>
                        <div class="card">
                            <div class="card-header" id="headingThree">
                                <h2 class="mb-0">
                                    <button class="btn btn-link collapsed" type="button" data-toggle="collapse"
                                        data-target="#collapseThree" aria-expanded="false"
                                        aria-controls="collapseThree">
                                        Kako mogu da otkazem rezervaciju?
                                    </button>
                                </h2>
                            </div>
                            <div id="collapseThree" class="collapse" aria-labelledby="headingThree"
                                data-parent="#accordionExample">
                                <div class="card-body">
                                    Ako ste direktno rezervisali preko VISER, posetite ovu stranicu da biste izmenili rezervaciju koja je u toku. Za
                                    sve rezervacije veb lokacija partnera, kontaktirajte odgovarajuću liniju za korisnike da biste promenili ili ažurirali
                                    rezervaciju.
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="col">
                    <label for="accordionExample2">
                        <h1 class="display-4 sub-heading">Uslovi iznajmljivanja</h1>
                    </label>

                    <div class="accordion" id="accordionExample2">
                        <div class="card">
                            <div class="card-header" id="headingFour">
                                <h2 class="mb-0">
                                    <button class="btn btn-link" type="button" data-toggle="collapse"
                                        data-target="#collapseFour" aria-expanded="true" aria-controls="collapseOne">
                                        Sta se od vas ocekuje
                                    </button>
                                </h2>
                            </div>

                            <div id="collapseFour" class="collapse show" aria-labelledby="headingFour"
                                data-parent="#accordionExample2">
                                <div class="card-body">
                                    Morate biti izmedju 21 i 80 godina starosti i imati punu vozacku dozvolu da bi iznajmili vozilo
                                </div>
                            </div>
                        </div>
                        <div class="card">
                            <div class="card-header" id="headingFive">
                                <h2 class="mb-0">
                                    <button class="btn btn-link collapsed" type="button" data-toggle="collapse"
                                        data-target="#collapseFive" aria-expanded="false" aria-controls="collapseTwo">
                                        Da li mogu sa probnom dozvolom da iznajmim vozilo?
                                    </button>
                                </h2>
                            </div>
                            <div id="collapseFive" class="collapse" aria-labelledby="headingFive"
                                data-parent="#accordionExample2">
                                <div class="card-body">
                                    Ne, vozaci koji su idalje pod probnom vozackom dozvolom nemaju pravo iznajmljivanja vozila
                                </div>
                            </div>
                        </div>
                        <div class="card">
                            <div class="card-header" id="headingSix">
                                <h2 class="mb-0">
                                    <button class="btn btn-link collapsed" type="button" data-toggle="collapse"
                                        data-target="#collapseSix" aria-expanded="false" aria-controls="collapseThree">
                                        Da li moze druga osoba samnom da iznajmi isto vozilo
                                    </button>
                                </h2>
                            </div>
                            <div id="collapseSix" class="collapse" aria-labelledby="headingSix"
                                data-parent="#accordionExample2">
                                <div class="card-body">
                                    Da, druga osoba takodje mora imati punu vozacku dozvolu, biti izmedju 21 i 80 godina starosti i biti prisutna sa vama pri preuzmanju vozila
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="w-100"></div>
                <div class="col">

                    <br><br>
                    <label for="accordionExample2">
                        <h1 class="display-4 sub-heading">Povratak vozila</h1>
                    </label>


                    <div class="accordion" id="accordionExample3">
                        <div class="card">
                            <div class="card-header" id="headingSeven">
                                <h2 class="mb-0">
                                    <button class="btn btn-link" type="button" data-toggle="collapse"
                                        data-target="#collapseSeven" aria-expanded="true" aria-controls="collapseSeven">
                                        Mogu li vratiti vozilo ako je lokacija van radnom vremena?
                                    </button>
                                </h2>
                            </div>

                            <div id="collapseSeven" class="collapse show" aria-labelledby="headingSeven"
                                data-parent="#accordionExample3">
                                <div class="card-body">
                                    Da. Svaka lokacija opremljena je ključem za povratak za sve povratke nakon radnog vremena. Imajte u vidu, međutim, da
                                    ćete i dalje biti odgovorni za stanje vozila dok ključevi ne budu primljeni i službeni na licu mesta obavljen zvaničan
                                    pregled tokom radnog vremena sledećeg jutra.
                                </div>
                            </div>
                        </div>
                        <div class="card">
                            <div class="card-header" id="headingEight">
                                <h2 class="mb-0">
                                    <button class="btn btn-link collapsed" type="button" data-toggle="collapse"
                                        data-target="#collapseEight" aria-expanded="false"
                                        aria-controls="collapseEight">
                                        Zelim zadrzati vozilo koji dan duze, da li je to moguce?
                                    </button>
                                </h2>
                            </div>
                            <div id="collapseEight" class="collapse" aria-labelledby="headingEight"
                                data-parent="#accordionExample3">
                                <div class="card-body">
                                    Da. Molimo zatražite produženje najma putem telefona ili e-maila na vašoj odgovarajućoj stanici. Potvrda svih izmena,
                                    uključujući ažuriranu cenu zakupa, biće vam dostavljena putem e-pošte.
                                </div>
                            </div>
                        </div>
                        <div class="card">
                            <div class="card-header" id="headingNine">
                                <h2 class="mb-0">
                                    <button class="btn btn-link collapsed" type="button" data-toggle="collapse"
                                        data-target="#collapseNine" aria-expanded="false" aria-controls="collapseNine">
                                        Zelim vratiti vozilo koji dan ranije, da li se postupak kaznjava?
                                    </button>
                                </h2>
                            </div>
                            <div id="collapseNine" class="collapse" aria-labelledby="headingNine"
                                data-parent="#accordionExample3">
                                <div class="card-body">
                                    Ne. Dobrodosli ste da vratite svoje vozilo pre planiranog roka. Međutim, ako je zakup unapred plaćen putem partnerske veb
                                    stranice, molimo vas da imate na umu da možda neće biti neophodno da vam se vratite za neiskorištene dane zakupa.
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="col">

                    <br><br>
                    <label for="accordionExample2">
                        <h1 class="display-4 sub-heading">Stanje vozila</h1>
                    </label>

                    <div class="accordion" id="accordionExample4">
                        <div class="card">
                            <div class="card-header" id="headingTen">
                                <h2 class="mb-0">
                                    <button class="btn btn-link" type="button" data-toggle="collapse"
                                        data-target="#collapseTen" aria-expanded="true" aria-controls="collapseOne">
                                        Smem li pusiti u vozilu?
                                    </button>
                                </h2>
                            </div>

                            <div id="collapseTen" class="collapse show" aria-labelledby="headingTen"
                                data-parent="#accordionExample4">
                                <div class="card-body">
                                    Ne. Pušenje u svim našim iznajmljenim vozilima je zabranjeno u svim okolnostima. Ako pušite u vozilu, naknada za
                                    čišćenje u odredjenom iznosu primjenjuje se na ukupne troškove iznajmljivanja.
                                </div>
                            </div>
                        </div>
                        <div class="card">
                            <div class="card-header" id="headingEleven">
                                <h2 class="mb-0">
                                    <button class="btn btn-link collapsed" type="button" data-toggle="collapse"
                                        data-target="#collapseEleven" aria-expanded="false" aria-controls="collapseTwo">
                                        Imate li posebnu politiku čišćenja?
                                    </button>
                                </h2>
                            </div>
                            <div id="collapseEleven" class="collapse" aria-labelledby="headingEleven"
                                data-parent="#accordionExample4">
                                <div class="card-body">
                                    Da. Ako vozilo zahteva prekomerno čišćenje ili posebne proizvode za čišćenje, naknada za čišćenje do odredjenog iznosa će se
                                    primeniti na ukupne troškove najma.
                                </div>
                            </div>
                        </div>
                        <div class="card">
                            <div class="card-header" id="headingTwelve">
                                <h2 class="mb-0">
                                    <button class="btn btn-link collapsed" type="button" data-toggle="collapse"
                                        data-target="#collapseTwelve" aria-expanded="false" aria-controls="collapseThree">
                                        Da li mogu da pređem granicu sa svojim iznajmljenim vozilom?
                                    </button>
                                </h2>
                            </div>
                            <div id="collapseTwelve" class="collapse" aria-labelledby="headingTwelve"
                                data-parent="#accordionExample4">
                                <div class="card-body">
                                    Mini, kompaktni i srednji časovi mogu putovati u sledeće zemlje bez ikakvih ograničenja:
                                    Andora, Belgija, Danska, Nemačka, Finska, Francuska, Gibraltar, Grčka, Velika Britanija, Irska, Italija, Lihtenštajn,
                                    Luksemburg, Malta, Monako, Holandija, Norveška, Austrija, Poljska, Portugal, San Marino, Švedska, Slovačka, Slovenija,
                                    Španija, Češka, Mađarska i Kipar. Za ove kategorije dozvoljen je i prolaz trajektom.
                                    
                                    Vozila standardne klase i više mogu SAMO putovati u sledeće zemlje:
                                    Andora, Belgija, Danska, Nemačka, Finska, Francuska, Velika Britanija, Irska, Italija, Lihtenštajn, Luksemburg, Monako,
                                    Holandija, Norveška, Austrija, Portugal, San Marino, Švedska i Španija. Prolaz trajektom nije dozvoljen za ove
                                    kategorije.
                                </div>
                            </div>
                        </div>
                    </div>

                </div>
            </div>
        </div>
    </div>




    <!-- FOOTER -->

    <footer id="footer" class="footer-1">
        <div class="main-footer widgets-dark typo-light">
            <div class="container">
                <div class="row">

                    <div class="col-xs-12 col-sm-6 col-md-4">
                        <div class="widget subscribe no-box">
                            <h5 class="widget-title">Rent-A-Car VISER<span></span></h5>
                            <p>
                                Renta a car VISER ima odličan odnos cene i kvaliteta. Mi se nalazimo u Beogradu, tačnije
                                na Novom
                                Beogradu.
                            </p>
                        </div>
                    </div>

                    <div class="col-xs-12 col-sm-6 col-md-4">
                        <div class="widget no-box">
                            <h5 class="widget-title">Linkovi<span></span></h5>
                            <ul class="thumbnail-widget">
                                <li>
                                    <div class="thumb-content"><a href="kontakt.php">Kontakt</a></div>
                                </li>
                                <li>
                                    <div class="thumb-content"><a href="ponude.php">Ponude</a></div>
                                </li>                                <li>
                                <div class="thumb-content"><a href="faq.php">FAQ</a></div>
                            </li>
                            </ul>
                        </div>
                    </div>

                    <div class="col-xs-12 col-sm-6 col-md-4">

                        <div class="widget no-box">
                            <h5 class="widget-title">Kontaktirajte nas<span></span></h5>

                            <p><a href="mailto:info@domain.com" title="glorythemes">rentviser@gmail.com</a></p>
                            <ul class="social-footer2">
                                <li class=""><a title="youtube" target="_blank" href="https://www.youtube.com/"><img
                                        alt="youtube" width="30" height="30"
                                        src="data:image/png;base64,"></a>
                                </li>
                                <li class=""><a href="https://www.facebook.com/" target="_blank" title="Facebook"><img
                                        alt="Facebook" width="30" height="30"
                                        src="data:image/png;base64,"></a>
                                </li>
                                <li class=""><a href="https://twitter.com" target="_blank" title="Twitter"><img
                                        alt="Twitter" width="30" height="30"
                                        src="data:image/png;base64,"></a>
                                </li>
                                <li class=""><a title="instagram" target="_blank" href="https://www.instagram.com/"><img
                                        alt="instagram" width="30" height="30"
                                        src="data:image/png;base64,"></a>
                                </li>
                            </ul>
                        </div>
                    </div>

                </div>
            </div>
        </div>

        <div class="footer-copyright">
            <div class="container">
                <div class="row">
                    <div class="col-md-12 text-center">
                        <p>Copyright Company Name ©
                            <script>document.write(new Date().getFullYear())</script>. All rights reserved.</p>
                    </div>
                </div>
            </div>
        </div>
    </footer>

    <!-- FOOTER END -->

    <div class="modal fade" tabindex="-1" id="loginModal" role="dialog">
        <div class="modal-dialog " role="document">
            <div class="modal-content bg-dark text-light">
                <div class="modal-header">
                    <h5 class="modal-title">Login</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <div class="modal-body">
                    <form action="faq.php" method="post">
                        <label for="korime">Korisničko ime</label>
                        <input type="text" class="form-control" id="korime" name="korime" placeholder="Unesite Vaše korisničko ime">
                        <label for="lozinka">Lozinka</label>
                        <input type="password" class="form-control" id="lozinka" placeholder="Unesite lozinku">
                    </form>
                    <div id="poruka"></div>
                </div>
                <div class="modal-footer">
                    <a href="registracija.php" class="text-light">Registracija</a>
                    <button type="button" class="btn" id="ulogujSe">Login</button>
                    <button type="button" class="btn" id="btnClose" data-dismiss="modal">Zatvori</button>
                </div>
            </div>
        </div>
    </div>


    <!-- Optional JavaScript -->
    <!-- jQuery first, then Popper.js, then Bootstrap JS -->
    <script src="https://code.jquery.com/jquery-3.3.1.slim.min.js"
        integrity="sha384-q8i/X+965DzO0rT7abK41JStQIAqVgRVzpbzo5smXKp4YfRvH+8abtTE1Pi6jizo"
        crossorigin="anonymous"></script>
    <script src="https://cdnjs.cloudflare.com/ajax/libs/popper.js/1.14.7/umd/popper.min.js"
        integrity="sha384-UO2eT0CpHqdSJQ6hJty5KVphtPhzWj9WO1clHTMGa3JDZwrnQq4sF86dIHNDz0W1"
        crossorigin="anonymous"></script>
    <script src="https://stackpath.bootstrapcdn.com/bootstrap/4.3.1/js/bootstrap.min.js"
        integrity="sha384-JjSmVgyd0p3pXB1rRibZUAYoIIy6OrQ6VrjIEaFf/nJGzIxFDsf4x0xIM+B07jRM"
        crossorigin="anonymous"></script>
    <script src="js/login.js"></script>

</body>

</html>